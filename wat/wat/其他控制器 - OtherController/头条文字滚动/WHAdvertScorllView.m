...
#import "WHAdvertScorllView.h"

static NSInteger const advertScrollViewTitleFont = 13;

#pragma mark - - - WHAdvertScrollViewStyleNormal 样式下的 cell
@interface WHAdvertScrollViewNormalCell : UICollectionViewCell
@property (nonatomic, strong) UIImageView *signImageView;
@property (nonatomic, strong) UILabel *titleLabel;
@end

@implementation WHAdvertScrollViewNormalCell
...
#pragma mark - - - WHAdvertScrollViewStyleMore 样式下的 cell
@interface WHAdvertScrollViewMoreCell : UICollectionViewCell
...
@implementation WHAdvertScrollViewMoreCell
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = [UIColor clearColor];
...
@interface WHAdvertScrollView () <UICollectionViewDelegate, UICollectionViewDataSource>
...
@implementation WHAdvertScrollView
...
- (void)initialization {
    ...
    [self addTimer];
    _advertScrollViewStyle = WHAdvertScrollViewStyleNormal;
}
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[WHAdvertScrollViewNormalCell class] forCellWithReuseIdentifier:advertScrollViewNormalCell];
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (self.advertScrollViewStyle == WHAdvertScrollViewStyleMore) {
        WHAdvertScrollViewMoreCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:advertScrollViewMoreCell forIndexPath:indexPath];
        NSString *topImagePath = self.imageArr[indexPath.item];
        if ([topImagePath hasPrefix:@"http"]) {
          //  [cell.topSignImageView sd_setImageWithURL:[NSURL URLWithString:topImagePath]];//网络图片
        ...
        if ([imagePath hasPrefix:@"http"]) {
           // [cell.bottomSignImageView sd_setImageWithURL:[NSURL URLWithString:imagePath]];//网络图片
        ...
        WHAdvertScrollViewNormalCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:advertScrollViewNormalCell forIndexPath:indexPath];
...
#pragma mark - - - setting
- (void)setAdvertScrollViewStyle:(WHAdvertScrollViewStyle)advertScrollViewStyle {
    _advertScrollViewStyle = advertScrollViewStyle;
    if (advertScrollViewStyle == WHAdvertScrollViewStyleMore) {
        _advertScrollViewStyle = WHAdvertScrollViewStyleMore;
        [_collectionView registerClass:[WHAdvertScrollViewMoreCell class] forCellWithReuseIdentifier:advertScrollViewMoreCell];
    }
}
